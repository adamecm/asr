...
"""
from matplotlib.artist import _finalize_rasterization
import numpy as np
import scipy.io
...
def b_func(o_t,mu_j,C_j,n=13):
  ...
  #inv_C_j = [1/x if x != 0 else 0 for x in C_j]
  #full_C_j = np.zeros((13,13))
  #inverse_C_j = np.zeros((13,13))
  #np.fill_diagonal(full_C_j,C_j)
  #np.fill_diagonal(inverse_C_j,inv_C_j)
  #print(inverse_C_j)
  #exit()
...
def beta_func(T,N,A_vector,b_vector):
  ...
  for t in range(T-1,-1,-1):
    if t==T-1:
      for i in range(1,N-1):
        betas[t][i] = A_vector[i][N-1]#*b_vector[N-1][j]
      print(betas[t])
    else:
      for i in range(1,N-1):
        for j in range(1,N-1):
