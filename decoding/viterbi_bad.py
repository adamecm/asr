...
min_path = np.argmin(acoustic_model[0])
...
for t in range(1,len(acoustic_model)):
  ...
  for w in range(len(phi_net)):
    ...
    for j in range(1,len(word)):
      ...
      minimum = min(prev_phi,current_phi)
    ...
    bigger = t if t < (len(phi_net[w])-1) else (len(phi_net[w])-1)
    ends.append(phi_net[w][bigger])
  min_path = np.argmin(ends)
  min_val = ends[min_path]
